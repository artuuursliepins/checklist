...
function getFiltered(){
  ...
  return state.items.filter(i => {
    ...
    if (a.section !== b.section) return a.section.localeCompare(b.section, "lv");
...
function render(){
  ...
  if (!items.length) {
    ...
    empty.textContent = "Nekas netika atrasts. Pamēģini noņemt filtrus vai mainīt meklējamo.";
    ...
    <div class="row">
      ...
      <div style="flex:1">
        <div class="itemTitle">
          ...
          <span class="chip status-chip ${statusClass(it.status)}">${escapeHtml(statusLabel(it.status))}</span>
